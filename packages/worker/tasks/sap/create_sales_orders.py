...
def create_sales_orders(
    ...
    if not po_working_network_path.exists():
        raise FileNotFoundError(
            f"PO working file not found at {po_working_network_path}"
        )
    ...
    for so_count, line_items in sales_orders.items():
        ...
        try:
            ...
            screenshot_path = f"{po_working_network_path.parent}/screenshot_{so_count}_{line_items[0].get('po number')}_{safe_filename(error_message)}.png"
            sap.session.take_screenshot(
                screenshot_path, window=0, format="png"
            )  # TODO: Take screenshot of native window instead of SAP's
            ...
            output_path = po_working_network_path.with_suffix(suffix)
